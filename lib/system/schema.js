...
module.exports = (m, env) => {
   ...
   if (user) {
      ...
      if (!('referralCount' in user)) user.referralCount = 0  // Number of times the referral code has been used
      if (!('referralCodeUsed' in user)) user.referralCodeUsed = false  // Whether the user has redeemed a referral code
      if (!('referredBy' in user)) user.referredBy = ''  // Track who referred this user
      if (!('referredUsers' in user)) user.referredUsers = []  // Users who joined using this referral code
      if (!isNumber(user.points)) user.points = 0  // Points of the user
