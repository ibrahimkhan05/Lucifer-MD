...
async function downloadFile(fileId, fileName, mimeType) {
    ...
    try {
        ...
        if (mimeType.startsWith('application/vnd.google-apps')) {
            // Handle Google Docs, Sheets, and Slides dynamically
...
exports.run = {
    ...
    async: async (m, { client, text, users, env, Func, Scraper }) => {
        ...
        const sizeInBytes = parseFloat(fileInfo.size || 0);
        if (isOversized(sizeInBytes, users, env)) {
        ...
        if (fileInfo.mimeType === 'application/vnd.google-apps.folder') {
            await client.reply(m.chat, '📂 Downloading folder...', m);
            ...
            for (const file of files) {
                ...
                if (isOversized(fileSize, users, env)) {
                    await client.reply(m.chat, `⚠️ Skipping ${file.name} (size: ${(fileSize / (1024 * 1024)).toFixed(2)} MB) as it exceeds the limit.`, m);
                    continue;
                }
