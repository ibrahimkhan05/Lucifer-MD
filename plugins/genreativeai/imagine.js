...
exports.run = {
    ...
    async: async (m, { client, text, command }) => {
        try {
            ...
            await handleModelGeneration(text, client, m);
        } catch (e) {
            console.error('Error:', e);
            return client.reply(m.chat, 'An error occurred. Please try again.', m);
        }
    },
    error: false,
    limit: true,
...
const handleModelGeneration = async (promptText, client, message) => {
    const model = 'dreamshaperXL10_alpha2.safetensors [c8afe2ef]';
