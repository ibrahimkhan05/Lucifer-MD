exports.run = {
    ...
    async: async (m, { client, args, isPrefix, command, Func }) => {
       try {
          ...
          if (!args || !args[0]) return client.reply(m.chat, Func.example(isPrefix, command, '8.8.8.8'), m);
          ...
          if (!json.status || !json.data) return client.reply(m.chat, Func.texted('bold', `🚩 IP address not found.`), m);
          ...
          // Include the footer and send the reply
          caption += global.footer;
          client.sendMessage(m.chat, caption, m);
