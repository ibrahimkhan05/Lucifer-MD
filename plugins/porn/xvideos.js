   ...
   async: async (m, { client, text, args, isPrefix, command, Func }) => {
      try {
         if (command === 'xvideos') {
            ...
            responseText += `To download a video, type: /getxvideos <number> or send the video URL directly.\nExample: /getxvideos 1 for the first video.`;
            ...
            if (text) {
               ...
               videoJson = await Func.fetchJson(`https://api.betabotz.eu.org/api/download/xvideosdl?url=${text}&apikey=beta-Ibrahim1209`);
            }
            // If the user selects a video from the search results
               ...
               videoJson = await Func.fetchJson(`https://api.betabotz.eu.org/api/download/xvideosdl?url=${selectedVideo.link}&apikey=beta-Ibrahim1209`);
            }
