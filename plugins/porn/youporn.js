...
// Function to handle the /youporn command (Search videos)
async function handleYouPornRequest(m, { client, text, isPrefix, command, Func }) {
    if (!text) return client.reply(m.chat, Func.example(isPrefix, command, 'step mom'), m);

    client.sendReact(m.chat, '🕒', m.key);

    // Fetch search results from YouPorn API
    const apiUrl = `https://lust.scathach.id/youporn/search?key=${text}`;
    try {
        ...
        let resultMessage = "*🎬 Y O U P O R N   S E A R C H*\n\nHere are the results for your search: " + text + ".\n\nPlease select a video from the list below. Reply with `/getyouporn <number>` to select a video.\n\n";
        data.data.forEach((v, index) => {
            resultMessage += `*${index + 1}*: ${v.title ? v.title : 'No title available'}\n\n`;
        });
...
// Function to handle the /getyouporn command (Fetch and list qualities)
async function handleGetYouPornCommand(m, { client, text }) {
    const index = parseInt(text.trim(), 10) - 1;
    const session = global.youPornSessions[m.chat];
...
async function handleGetYtdlCommand(m, { client, text }) {
    ...
    if (!session) {
        return client.reply(m.chat, "❌ No active session. Please start with /youporn command first.", m);
    }
...
exports.run = {
    ...
    async: async (m, { client, text, isPrefix, command, Func }) => {
        try {
            if (command === 'youporn') {
                await handleYouPornRequest(m, { client, text, isPrefix, command, Func });
            } else if (command === 'getyouporn') {
                await handleGetYouPornCommand(m, { client, text });
